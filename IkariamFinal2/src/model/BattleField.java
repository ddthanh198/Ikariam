...
public class BattleField {
    ...
    private int numberOfSlotForHI;   // số ô cho hạng nặng  
    private int numberOfSlotForLRF;  // số ô cho hàng 2
    private int numberOfSlotForLI;   // số ô cho cánh
    ...
    public void setArtilleryClass() {
        int index = countByType(artillery, Army.Unit.Mortar);     // index là số slot có thể xếp mortar
        int slotForMortar = (int) Math.ceil((reserve.getMortar().size() * Mortar.size) / (float) this.spacePerSlotForAr);  // số slot cần cho mortar
    ...
    // đếm ố slot có loại quân unitType
