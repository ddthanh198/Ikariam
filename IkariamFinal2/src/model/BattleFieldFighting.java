...
public class BattleFieldFighting {
    ...
    public void startAttack() {
        ...
        int timeOneRound = 15 * 60;
        int timeRun = timeOneRound;
        while(true){
            System.out.println("waiting ...  " + timeRun);
            if(timeRun < 0){
                break;
            }
            timeRun -= BattleFieldFighting.xSpeed;
            try {
                Thread.sleep(1000);
            } catch (InterruptedException ex) {
                Logger.getLogger(BattleFieldFighting.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        
        int houseID = this.houseID;

        if (totalHitpointDefenceAfter == totalHitpointDefenceBefore) {                     // trận đấu kết thúc
            JOptionPane.showMessageDialog(null, "Enemy " + IsLandUI.house[houseID].getName() + " won");
            System.out.println("won this");
        } else {
            attackBattleField.setUnitsBackToReserve();                 // chuyển quân về dự trữ
            defenceBattleField.setUnitsBackToReserve();

            for (int j = 0; j < IsLandUI.house[houseID].getWaitingWaveAttack().size(); j++) {       // lấy quân từ dữ trữ ra
                Army army = new Army();
    ...
    public void startAttack2() {
            ...
            public void run() {
                if (totalHitpointDefenceAfter == totalHitpointDefenceBefore) {                     // trận đấu kết thúc
                    ...
                    for (int j = 0; j < IsLandUI.house[houseID].getWaitingWaveAttack().size(); j++) {       // lấy quân từ dữ trữ ra
                        ...
                        }, 15000);    // 15s cập nhật quân đội 1 lần                   
                    }
        ...
        System.out.println("xong 1 hiep");
    }
