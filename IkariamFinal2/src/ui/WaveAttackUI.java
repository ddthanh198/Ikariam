...
import static ui.IsLandUI.myHouse;

public class WaveAttackUI extends JDialog {
    public static WaveAttackUI waveAttackUI = new WaveAttackUI("WaveAttack", IsLandUI.myHouse.getSentRealArmy(), IsLandUI.myHouse.getSendingArmy());
    ...
    private void addControls() {
        ...
        pnXSpeed = new PnXSpeed();
        pnXSpeed.setBounds(10, 10 , 180 , 30);
        
        pnMain.add(pnXSpeed);
        
        for (int i = 0; i < 7; i++) {
            lblTitle[i] = new JLabel();
            lblTitle[i].setBounds(30 + 110 * i, 40, 100, 50);
            ...
            for (int j = 0; j < arrList.size(); j++) {
                ...
                if (numberOfUnit != 0) {
                    ...
                    Timer dongho = new Timer(1000, new ActionListener() {
                        long timeOneRound = 15 * 60 * 1000;
                        long timeRun = timeOneRound;
                        public void actionPerformed(ActionEvent e) {
                            pnWave[temp].getLblArrivalTime().setText(convertSecToMMss(timeRun));
                            timeRun -= BattleFieldFighting.xSpeed * 1000;
                            if(timeRun < 0){
                                timeRun = timeOneRound;
                            }
                        }
                    });
        ...
        sendingArmy.forEach((key, queue) -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (numberOfUnit != 0) {
                    ...
                    Timer dongho = new Timer(1000, new ActionListener() {
                        long timeTravel = sendingArmy.getFinishTime() - sendingArmy.getStartTime();
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            pnWave[temp].getLblArrivalTime().setText(convertSecToMMss(timeTravel));
                            timeTravel -= BattleFieldFighting.xSpeed * 1000;
    ...
    private String convertSecToMMss(long miniSec){
        String result = "";
        if(miniSec <= 0){
            result = "00:00";
        }else{
            long min = miniSec/1000/60;
            long sec = miniSec/1000%60;
            result = min + ":" +sec;
        }
        return result;
    }
